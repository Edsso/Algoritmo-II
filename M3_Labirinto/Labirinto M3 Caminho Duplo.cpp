...
void leituraDoLabirinto(vector<string> &labirinto, Jogadas &jogada)
{
    ...
    do
    {
        ...
        if (arquivoEntrada.is_open())
        {
            ...
            //verificar caminhos duplos

            for (int i = 1; i < contadorLinha-1; i++){
                for (int j=1; j < contadorColuna-1; j++){

              if ( labirinto[i][j] == '_' && labirinto[i][j-1] == '_' && labirinto[i+1][j] == '_' && labirinto[i+1][j-1] == '_'){
                    cout << "Possui Caminho Duplo" << endl;
                    break;
                }
                if ( labirinto[i][j] == '_' && labirinto[i+1][j] == '_' && labirinto[i][j+1] == '_' && labirinto[i+1][j+1] == '_'){
                    cout << "Possui Caminho Duplo" << endl;
                    break;
                }
                if ( labirinto[i][j] == '_' && labirinto[i-1][j] == '_' && labirinto[i][j-1] == '_' && labirinto[i-1][j-1] == '_'){
